...
func TestUDPFragmentReassemblyAndMTU(t *testing.T) {
	...
	}}, func(o *tun.StackOptions) { o.TunOptions.MTU = 1280 })
	...
	for _, part := range []struct {
		...
		binary.BigEndian.PutUint16(fragment[10:], checksum(fragment[:20]))
		d.in <- fragment
	}
	select {
	case got := <-received:
...
func TestInvalidOptionsAndStartFailure(t *testing.T) {
	d := newMemoryTun()
	defer d.Close()
	base := tun.StackOptions{Tun: d, Handler: &testHandler{}, TunOptions: tun.Options{MTU: 1500, Inet4Address: []netip.Prefix{netip.MustParsePrefix("198.18.0.1/30")}}}
	for _, change := range []func(*tun.StackOptions){func(o *tun.StackOptions) { o.Tun = nil }, func(o *tun.StackOptions) { o.Handler = nil }, func(o *tun.StackOptions) { o.TunOptions.MTU = 65536 }, func(o *tun.StackOptions) { o.TunOptions.Inet4Address = nil }} {
		options := base
		change(&options)
	...
	s.(*Stack).options.TunOptions.MTU = 65536
	if err = s.Start(); err == nil {
		t.Fatal("invalid stack configuration started")
	}
	if s.(*Stack).stack != nil {
