...
type testHandler struct {
	...
	prepare func(tun.DirectRouteContext) (tun.DirectRouteDestination, error)
}
...
func (h *testHandler) PrepareConnection(_ string, _, _ M.Socksaddr, writer tun.DirectRouteContext, _ time.Duration) (tun.DirectRouteDestination, error) {
	if h.prepare != nil {
...
func testStack(t *testing.T, device tun.Tun, handler *testHandler, modify func(*tun.StackOptions)) *Stack {
	t.Helper()
	options := tun.StackOptions{Tun: device, Handler: handler, Context: context.Background(), ICMPTimeout: 50 * time.Millisecond,
		TunOptions: tun.Options{MTU: 1500, Inet4Address: []netip.Prefix{netip.MustParsePrefix("198.18.0.1/30")}, Inet6Address: []netip.Prefix{netip.MustParsePrefix("fd00::1/126")}}}
	if modify != nil {
		modify(&options)
	}
	v, err := New(options)
...
func transportPacket(source, target netip.Addr, protocol byte, payload []byte) []byte {
	...
	binary.BigEndian.PutUint16(payload[offset:], checksum(append(pseudo, payload...)))
	...
	for _, pair := range [][2]string{{"198.18.0.1", "8.8.8.8"}, {"fd00::1", "2001:4860:4860::8888"}} {
		t.Run(pair[0], func(t *testing.T) {
			...
			for _, port := range []uint16{53, 443} {
				...
				src, dst, proto, ok := packetAddresses(response)
...
func TestICMPEchoAndFilter(t *testing.T) {
	for _, pair := range [][2]string{{"198.18.0.1", "8.8.8.8"}, {"fd00::1", "2001:4860:4860::8888"}} {
		t.Run(pair[0], func(t *testing.T) {
			...
			d.in <- p
			response := readPacket(t, d)
			src, dst, _, ok := packetAddresses(response)
	...
	d := newMemoryTun()
	testStack(t, d, &testHandler{}, func(o *tun.StackOptions) {
	...
	d.in <- p
	r := readPacket(t, d)
	src, dst, _, _ := packetAddresses(r)
	if src.String() != "10.0.0.1" || dst != source || checksum(r[:20]) != 0 {
